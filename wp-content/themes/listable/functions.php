...
function listable_scripts() {
	...
	if ( '' == pixelgrade_option( 'mapbox_token', '' ) ) {
		...
		wp_enqueue_script( 'google-maps', '//www.google.cn/maps/api/js?v=3.exp&amp;libraries=places' . $google_maps_key, array(), '3.22', true );
		$listable_scripts_deps[] = 'google-maps';
	} elseif ( wp_script_is( 'google-maps' ) || listable_using_facetwp() ) {
		wp_deregister_script('google-maps');
		wp_enqueue_script( 'google-maps', '//www.google.cn/maps/api/js?v=3.exp&amp;libraries=places' . $google_maps_key, array(), '3.22', false );
		$listable_scripts_deps[] = 'google-maps';
	}
	...
	if ( ( isset( $post->post_content ) && has_shortcode( $post->post_content, 'jobs' ) && true === listable_jobs_shortcode_get_show_map_param( $post->post_content ) )
        ...
        wp_enqueue_script( 'baidumapjs', 'http://api.map.baidu.com/api?v=2.0&ak=BrROQptLxvqmnN5UG505lUNo1hMAiFd9', array( 'jquery' ), '1.0.0', true );
        $listable_scripts_deps[] = 'baidumapjs';
    }
